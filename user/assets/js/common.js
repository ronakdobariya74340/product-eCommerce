...
$(document).ready(function() {
    $('#recordPerPage').val(common_filter.itemPerPage);
...
$(document).on("input", "#access_code", function() {
    $(".error-msg").addClass("d-none");
});

$(document).on("keydown", "#access_code", function(e){
    if (e.key === "Enter" || e.which === 13) {
        e.preventDefault();
        if (!$(this).data("enterPressed")) {
            $(this).data("enterPressed", true);
            $("#submit_access_code").trigger("click");
        };
    };
});
...
$(document).on('click', '.page_no', function(e) {
    e.preventDefault();
    const cp = parseInt($(this).data("page"));
    const table = $(this).data("table");

    common_filter.currentPage = cp;

    const furl = multipleFilter[table]['filter_url'];
...
function changeRecordPerPage(url, table) {
    ...
    if (isNaN(recPp)) {
        ...
        common_filter.itemPerPage = 10;
        $("#recordPerPage").val(10);
    } else if (recPp < 1) {
        return false;
    } else {
        common_filter.itemPerPage = parseInt(recPp);
    };

    common_filter.currentPage = 1;
...
async function filterData(url, table) {
    ...
    var flush = 1;
    if (typeof multipleFilter[table] !== 'undefined' && typeof multipleFilter[table]['common_filter'] !== 'undefined') {
        flush = 0;
        $.each(multipleFilter[table]['common_filter'], function(k, v) {
            if (typeof common_filter[k] === 'undefined') {
                common_filter[k] = v;
            };
        });
    } else {
    ...
    await $.ajax({
        ...
        success: function(res) {
            if (res.flag === 0) {
                common_filter.totalItems = 0;
                common_filter.totalPages = 0;
                $("#" + table).html("");
                $(".pagination").html("");
            } else {
                $("#" + table).html(res.blade);
                common_filter.totalItems = res["total_record"];

                if( common_filter.totalItems <= 10){
                    $('.pagination-area').addClass('d-none');
                } else {
                    $('.pagination-area').removeClass('d-none');
                };

                common_filter.totalPages = common_filter.totalItems > 0 ? Math.ceil(common_filter.totalItems / common_filter.itemPerPage) : 0;
                if(common_filter.totalPages > 0){
            ...
            multipleFilter[table]['common_filter'] = common_filter;
...
function setFilters(searchObject, removeField) {
    common_filter = {...common_filter, ...searchObject};

    if(removeField) {
        common_filter[removeField] = "";
    };
    common_filter.currentPage = 1;
};
...
function resetFilters(searchObject, table) {
    if (typeof table !== "undefined" && typeof multipleFilter[table] !== "undefined") {
        multipleFilter[table]["filtcommon_filterers"] = common_filter;
    };

    common_filter = {...common_filter, ...searchObject};
    common_filter.currentPage = 1;

    if (common_filter?.multiselect?.length == 0) {
        delete common_filter.multiselect;
    };
};
...
function flushFilters(keep) {
    if (keep) {
        common_filter = {
            totalItems: 0,
            itemPerPage: common_filter.itemPerPage,
            currentPage: 1,
            totalPages: 1,
        };
    } else {
...
function paginationInput(e, cp, tp, table) {
    ...
    common_filter.currentPage = newPage;
    var furl = multipleFilter[table]["filter_url"];
    filterData(furl, table);
};

function setPagination(table) {
    var tp = common_filter.totalPages;
    var cp = common_filter.currentPage;
    ...
    // Prev button
    html += cp === 1
    ? `<li class="page-item disabled"><a class="page-link text" tabindex="-1">Prev</a></li>`
    : `<li class="page-item"><a class="page-link text page_no" data-page="${cp-1}" data-table="${table}">Prev</a></li>`;


    // Page 1
    html += `<li class="page-item"><a class="page-link number ${cp===1?'active':''} page_no" data-page="1" data-table="${table}">1</a></li>`;

    function appendEllipsis() {
        html += `<li class="page-item dot-view"><a class="page-link number border-0">...</a></li>`;
    }

    if (tp <= 7) {
        for (let i = 2; i <= tp; i++) {
            html += `<li class="page-item"><a class="page-link number ${cp===i?'active':''} page_no" data-page="${i}" data-table="${table}">${i}</a></li>`;
        }

    } else {
        if (cp <= 4) {
        for (let i = 2; i <= 5; i++) html += `<li class="page-item"><a class="page-link number ${cp===i?'active':''} page_no" data-page="${i}" data-table="${table}">${i}</a></li>`;
        appendEllipsis();
        }
        else if (cp > tp - 4) {
        appendEllipsis();
        for (let i = tp - 4; i <= tp - 1; i++) html += `<li class="page-item"><a class="page-link number ${cp===i?'active':''} page_no" data-page="${i}" data-table="${table}">${i}</a></li>`;
        }
        else {
        appendEllipsis();
        for (let i = cp - 1; i <= cp + 1; i++) html += `<li class="page-item"><a class="page-link number ${cp===i?'active':''} page_no" data-page="${i}" data-table="${table}">${i}</a></li>`;
        appendEllipsis();
        }
        // Last page
        html += `<li class="page-item"><a class="page-link number ${cp===tp?'active':''} page_no" data-page="${tp}" data-table="${table}">${tp}</a></li>`;
    }

    // Next button
    html += cp === tp
    ? `<li class="page-item disabled"><a class="page-link text">Next</a></li>`
    : `<li class="page-item"><a class="page-link text page_no" data-page="${cp+1}" data-table="${table}">Next</a></li>`;
    ...
    $("#recordPerPage" + id).find("option[value='" + common_filter.itemPerPage + "']").attr("selected", true);

    if (cp == 1) {
        $(cls2 + ".pagination li:first-child").removeClass("page_no").addClass("disabled");
        $(cls2 + ".pagination li:nth-child(2)").removeClass("page_no").addClass("disabled");
    };

    if (cp == tp) {
        $(cls2 + ".pagination li:last-child").removeClass("page_no").addClass("disabled");
        $(cls2 + ".pagination li:nth-last-child(2)").removeClass("page_no").addClass("disabled");
        $(cls2 + ".pagination li .pagination-div-tag").addClass("disabled");
    };
};
...
function showToast(flag, val, time = 3000) {
    if (!val) return;

    // Ensure toast wrapper exists
    const wrapperSelector = ".tost-wrapper";
    if ($(wrapperSelector).length === 0) {
        $("body").append('<div class="tost-wrapper"></div>');
    }

    const wrapper = $(wrapperSelector);

    // Remove all existing toasts immediately (no animation)
    wrapper.find(".tost-container").remove();

    // Toast config
    const toastTypes = {
        1: { class: "tost-success", icon: "/images/icon-park.svg", title: "Success!" },
        0: { class: "tost-error", icon: "/images/icon-w-info.png", title: "Error!" },
        2: { class: "tost-error", icon: "/images/icon-w-info.png", title: "Error!" },
        // default: { class: "tost-warning", icon: "/images/icon-warning.svg", title: "Warning!" }
    };

    const config = toastTypes[flag] || toastTypes.default;
    const toastId = `toast-${Date.now()}`;

    // Toast HTML
    const toastHTML = `
        <div class="tost-container ${config.class}" id="${toastId}">
            <div class="tost-icon">
    ...
    // Add toast to top
    wrapper.prepend(toastHTML);

    // Auto-remove after timeout (no hide class or animation)
    setTimeout(() => {
        $(`#${toastId}`).remove();
    }, time);
}
